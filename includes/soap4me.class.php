...
class Soap4me
{
	...
	private function findEpisodes()
	{
		...
		foreach ($eps as $ep) {
			...
			$episodes[$serial][$number][$quality] = [
				'serial' => $serial,
				'season' => $season,
				'number' => $number,
				'name' => $name,
		...
		foreach ($eps as $serial) {
			foreach ($serial as $episod) {
				...
				if (!empty($res['ok'])) {
					...
					$wget = `which wget`;
					exec(trim($wget) . ' --no-check-certificate --random-wait -t 10 --retry-connrefused  -O ' . escapeshellarg($episod['path']) . ' ' . $url,
						$output, $retvar);
	...
	{
		return sprintf(' %s / Season %s / %s ', $episode['serial'], $episode['season'],
	...
	private function emailed($episod)
	{
		...
		file_get_contents('https://api.mailgun.net/v2/' . SomeShit::$config['mailgun']['domain'] . '/messages', false,
			$stream);
	}
	...
	private function filter($episodes)
	{
		foreach ($episodes as $serial_name => $eps) {
			foreach ($eps as $ep => $episod) {
				$qs = array_keys($episod);

				if(in_array('fullHD', $qs)){
					$res = 'fullHD';
				} elseif ( in_array('HD', $qs)){
					$res = 'HD';
				} else {
					$res = 'SD';
				}

				$episodes[$serial_name][$ep] = array_filter($episodes[$serial_name][$ep], function($v) use ($res) {
					return ($v['quality'] === $res);
				});
			}
